...
# Queues for inter-thread and inter-coroutine communication
    ...
    while not stop_event.is_set():
        try:
            ...
            if key.strip().lower() == 'q':
                ...
                await broadcast_queue.put("New Talk")
...
# This is the synchronous function for translation that will run in a thread
            ...
            for response in responses:
                ...
                for result in response.results:
                    if result.is_final:
                        ...
                        # Pass the final text to the async loop
                        loop.call_soon_threadsafe(broadcast_queue.put_nowait, original_text)
        except Exception as e:
...
# This function now handles both translation and broadcasting
async def broadcast_loop(executor):
    while not stop_event.is_set():
        try:
            original_text = await broadcast_queue.get()
            if original_text == "New Talk":
                translated_text = "New Talk"
            else:
                # Perform the blocking translation in the executor
...
async def main():
    ...
    broadcast_task = asyncio.create_task(broadcast_loop(executor))
